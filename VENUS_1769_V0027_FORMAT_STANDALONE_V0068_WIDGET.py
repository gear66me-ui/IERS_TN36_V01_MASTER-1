...
OUT = Path("/content/VENUS_1769_V0068_WIDGET_OUTPUT")
PNG = OUT / "VENUS_1769_V0068_WIDGET.png"
CSV = OUT / "VENUS_1769_V0068_CONTACTS_GEOMETRY.csv"
...
def fit(pts):
    m = pts.mean(axis=0)
    u, sv, vt = np.linalg.svd(pts - m, full_matrices=False)
    d = vt[0]
    if d[0] < 0:
        d = -d
...
def draw_events(ax, st, evs, main):
    ...
    for ev in evs:
        p = st["epts"][ev]
        disk(ax, p, st["r"][ev][1], site["color"])
...
def make_plot(pv, va, g):
    ...
    for st in (pv, va):
        ax.plot(st["pts"][:,0], st["pts"][:,1], c=st["site"]["color"], lw=.38, label=st["site"]["label"])
    ...
    for zax, evs, title in ((eg,("C3","C4"),"EGRESS ZOOM — C3 / C4 TANGENCY"), (ing,("C1","C2"),"INGRESS ZOOM — C1 / C2 TANGENCY")):
        ...
        for st in (pv, va):
            zax.plot(st["pts"][:,0], st["pts"][:,1], c=st["site"]["color"], lw=.38)
    ...
    fig.text(.5,.02,"NO AI IMAGES — Matplotlib only. JPL Horizons geometric ecliptic vectors; diameters from stated reduction constants.",ha="center",fontsize=6,color=MUTED)
