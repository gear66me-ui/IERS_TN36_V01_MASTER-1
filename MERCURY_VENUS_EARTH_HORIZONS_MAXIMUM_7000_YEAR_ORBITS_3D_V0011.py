# V0011
# Audit reference: JPL Horizons online-service maximum future 3D orbit bundle for Mercury, Venus, and Earth from 2026 through 9026 at 1,000-year intervals.
...
STEP = "1d"
OUT = Path("/content/MERCURY_VENUS_EARTH_HORIZONS_MAXIMUM_7000_YEAR_ORBITS_3D_V0011_OUTPUT")
PNG = OUT / "MERCURY_VENUS_EARTH_HORIZONS_MAXIMUM_7000_YEAR_ORBITS_3D_V0011.png"
CSV = OUT / "MERCURY_VENUS_EARTH_HORIZONS_MAXIMUM_7000_YEAR_ORBITS_3D_V0011.csv"
...
def make_plot(orbits: dict[str, list[dict]], summaries: list[dict]) -> None:
    ...
    for name in ("Mercury", "Venus", "Earth"):
        ...
        for index, orbit in enumerate(orbits[name]):
            ...
            width = 0.55 + 0.65 * index / max(1, epoch_count - 1)
            ...
            for marker_name, key, marker in [
                ...
                if index in (0, epoch_count - 1):
                    ax.text(
                        ...
                        color=cfg["color"], fontsize=7.8, weight="bold", zorder=8,
                    )
...
def main() -> None:
    ...
    section("COMMENTS")
    print("The prior 17026 target is REJECTED because the online Horizons service stops at A.D. 9999.")
    print("This widget uses the maximum valid future millennium series: 2026 through 9026.")
    ...
    print("The A.D. 10026–17026 requests from V0010 are REJECTED by Horizons and are not plotted.")
    ...
    print("VERIFIED all plotted coordinates use JPL heliocentric ecliptic vectors in million kilometers")
