...
SOLAR_FILL_COLOR = "#D95A1B"
SOLAR_FILL_ALPHA = 0.260
...
def patch_solar_constants(source: str) -> str:
    ...
    new_width = float(width_match.group(1)) * LIMB_WIDTH_FACTOR
    source = re.sub(
...
def patch_venus_disk_paint_lines(source: str) -> str:
    old = '''def add_venus_disk(
    axis: plt.Axes,
    center: np.ndarray,
    radius_arcsec: float,
    color: str,
...
'''
    ...
    axis.add_patch(
        Circle(
            ...
            edgecolor=color,
            alpha=0.98,
            linewidth=DISK_LINE_WIDTH * VENUS_PAINT_LINE_FACTOR,
            zorder=7,
        )
    )
'''
    if old not in source:
        raise RuntimeError("Venus disk block was not found; no unsafe partial patch applied.")
    return source.replace(old, new, 1)
...
''',
    ...
    vardo_angle = float(vardo_result["fit"]["angle_deg"])
    average_angle = 0.5 * (point_angle + vardo_angle)
    delta_angle = abs(vardo_angle - point_angle)
    rows = [
        ["Quantity", "Definition", "Arcseconds", "Kilometers"],
''',
        1,
    )
    old_row = '''        ["ᾱ", "Average track angle (degrees)", f"{average_angle:.6f}°", ""],'''
    new_row = '''        ["Δα", "Delta track angle, |αV − αPV| (degrees)", f"{delta_angle:.6f}°", ""],'''
...
def build_v0079_source() -> str:
    ...
    source = patch_venus_disk_paint_lines(source)
...
def main() -> None:
    source = build_v0079_source()
    namespace = {"__name__": "__main__", "__file__": "VENUS_1769_V0027_FORMAT_STANDALONE_V0079.py"}
    exec(compile(source, "VENUS_1769_V0027_FORMAT_STANDALONE_V0079.py", "exec"), namespace, namespace)
