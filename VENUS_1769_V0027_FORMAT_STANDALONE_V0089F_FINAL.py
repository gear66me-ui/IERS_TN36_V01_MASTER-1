# V0089F
# Audit reference: final standalone V0089E-derived publication widget; complete Python/Matplotlib/JPL implementation; footer includes GitHub file address and curl command; no AI images.
...
FILE_NAME = "VENUS_1769_V0027_FORMAT_STANDALONE_V0089F_FINAL.py"
REPO_PATH = FILE_NAME
...
OUT = ROOT / "VENUS_1769_V0089F_FINAL_OUTPUT"
PNG = OUT / "VENUS_1769_V0089F_FINAL_900DPI.png"
PDF = OUT / "VENUS_1769_V0089F_FINAL_VECTOR.pdf"
SVG = OUT / "VENUS_1769_V0089F_FINAL_VECTOR.svg"
CSV = OUT / "VENUS_1769_V0089F_FINAL.csv"
DPI = 900
...
for import_name, package_name in (
    ("numpy", "numpy"), ("pandas", "pandas"), ("scipy", "scipy"), ("astropy", "astropy"),
    ("astroquery", "astroquery"), ("matplotlib", "matplotlib"), ("IPython", "ipython"),
):
...
def plot(pv: dict, va: dict, g: dict, max_resid: float) -> None:
    ...
    rr = [["Quantity", "Symbol", "Value", "Unit / status"], ["IAU 1976 solar horizontal parallax", "π₀", f"{pi0:.6f}", "arcsec"], ["IAU 1976 full precision", "π₀", f"{pi0:.12f}", "arcsec"], ["A′B′ common-normal separation", "A′B′", f"{g['apbp_as']:.6f}", "arcsec"], ["A′B′ common-normal separation", "A′B′", f"{g['apbp_km']:,.6f}", "km"], ["AB projected baseline", "AB", f"{g['ab_as']:.6f}", "arcsec"], ["AB projected baseline", "AB", f"{g['ab_km']:,.6f}", "km"], ["Average track angle", "ᾱ", f"{0.5 * (pv['fit']['angle'] + va['fit']['angle']):.6f}", "deg"]]
    ...
    footer_1 = "NO AI IMAGES — Python/Matplotlib/JPL Horizons only. Final V0089E-derived publication widget."
    footer_2 = f"GitHub: {GITHUB_URL} | Raw: {RAW_URL} | File: {FILE_NAME} | Run: {CURL_COMMAND} ; {RUN_COMMAND}"
    fig.text(0.5, 0.037, footer_1, ha="center", fontsize=5.8, color=MUTED)
    fig.text(0.5, 0.019, footer_2, ha="center", fontsize=4.8, color=MUTED)
...
def main() -> None:
    ...
    print("COMMENTS")
    print("Final standalone V0089E-derived publication plot. Contact C1/C2 rows use the light-blue table color. Footer embeds permanent GitHub retrieval information. No AI images.")
    m = master_frame()
    c = cache_build(m)
    pv = station(c, PV)
    va = station(c, VA)
    ...
    print("PASS: V0089F is complete standalone Python/Matplotlib/JPL code; V0089E light-blue contact table style preserved; GitHub source footer embedded in plot.")
