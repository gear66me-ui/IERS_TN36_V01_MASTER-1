# V0030
# Audit reference: JPL Earth orbital-plane precession speed, direction, and signed-curvature audit around the A.D. 3000-4500 cross-track turning point.
...
OUT = Path("/content/EARTH_JPL_PRECESSION_SHIFT_SPEED_CURVATURE_AUDIT_V0030_OUTPUT")
DATA_CSV = OUT / "EARTH_JPL_PRECESSION_SPEED_CURVATURE_V0030.csv"
SUMMARY_CSV = OUT / "EARTH_JPL_PRECESSION_SHIFT_SUMMARY_V0030.csv"
PNG_PATH = OUT / "EARTH_JPL_ORBITAL_NORMAL_PATH_V0030.png"
PNG_SPEED = OUT / "EARTH_JPL_PRECESSION_SPEED_COMPONENTS_V0030.png"
PNG_DIRECTION = OUT / "EARTH_JPL_TANGENT_DIRECTION_CURVATURE_V0030.png"
PNG_TABLE = OUT / "EARTH_JPL_PRECESSION_SHIFT_TABLE_V0030.png"
CACHE_CSV = OUT / "EARTH_JPL_ANNUAL_STATES_3000_4500_V0030.csv"
...
def query_states() -> pd.DataFrame:
    ...
    if V0027_FOCUS.exists():
        cached = pd.read_csv(V0027_FOCUS)
        required = {"astronomical_year", "x", "y", "z", "vx", "vy", "vz"}
        if required.issubset(cached.columns) and len(cached) >= 1490:
...
def derive_geometry(states: pd.DataFrame) -> pd.DataFrame:
    ...
    radius_deg = np.divide(1.0, np.abs(signed_curvature), out=np.full_like(signed_curvature, np.nan), where=np.abs(signed_curvature) > 0.0)
...
def find_event_summary(data: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    ...
    valid = np.isfinite(data["total_speed_smooth_arcsec_per_century"].to_numpy(dtype=float))
    ...
    present_like_index = int(np.argmin(np.abs(years - 3760.0)))
    ...
    summary = {
        ...
        "speed_at_3760_arcsec_per_century": float(total_speed[present_like_index]),
        "tangent_direction_at_3760_deg": float(tangent[present_like_index]),
        "signed_curvature_at_3760_per_deg": float(curvature[present_like_index]),
    }

    speed_drop_fraction = (
        summary["maximum_total_speed_arcsec_per_century"] - summary["minimum_total_speed_arcsec_per_century"]
    ) / summary["maximum_total_speed_arcsec_per_century"]
    summary["speed_range_percent"] = speed_drop_fraction * 100.0
    summary["verdict"] = (
        "DIRECTION SHIFT WITHOUT MAJOR SLOWDOWN"
        if np.isfinite(cross_turn_year) and speed_drop_fraction < 0.05
    ...
    rows = [
        ...
        {"quantity": "Speed variation", "value": f"{summary['speed_range_percent']:.6f} %"},
        ...
        {"quantity": "Verdict", "value": summary["verdict"]},
    ]
...
def plot_path(data: pd.DataFrame, summary: dict) -> None:
    ...
    year_values = sample["astronomical_year"].to_numpy(dtype=float)
    for target in [3000, 3400, 3760, 4100, 4500]:
        index = int(np.argmin(np.abs(year_values - target)))
...
def plot_direction(data: pd.DataFrame, summary: dict) -> None:
    ...
    if np.isfinite(summary["cross_track_turn_year"]):
        ax.axvline(summary["cross_track_turn_year"], linewidth=0.7, linestyle="--", alpha=0.85)
    if np.isfinite(summary["curvature_sign_change_year"]):
        ax.axvline(summary["curvature_sign_change_year"], linewidth=0.6, linestyle=":", alpha=0.85)
    ...
    legend = ax.legend(handles1 + handles2, labels1 + labels2, frameon=False, fontsize=9, loc="best")
    for text in legend.get_texts():
        text.set_color("white")
    fig.tight_layout()
