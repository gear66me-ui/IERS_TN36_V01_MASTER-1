# V0093
# Audit reference: standalone JPL Horizons A-prime/B-prime fixed-vector versus instantaneous tangent-normal comparison; Matplotlib only.
...
OUT = Path("/content/VENUS_1769_APRIME_BPRIME_VECTOR_VS_INSTANT_V0093_OUTPUT")
PNG = OUT / "VENUS_1769_APRIME_BPRIME_VECTOR_VS_INSTANT_V0093.png"
CSV = OUT / "VENUS_1769_APRIME_BPRIME_VECTOR_VS_INSTANT_V0093.csv"
CONTACT_CSV = OUT / "VENUS_1769_APRIME_BPRIME_VECTOR_VS_INSTANT_CONTACTS_V0093.csv"
...
def fixed_apbp(c: dict, jd: float, g: dict) -> dict[str, float]:
...
def plot(df: pd.DataFrame, evs: dict[str, dict[str, float]], stats: dict[str, float | str]) -> None:
    ...
    fig.text(0.5, 0.043, f"File: VENUS_1769_APRIME_BPRIME_VECTOR_VS_INSTANT_V0093.py | Output: {PNG.name} | CSV: {CSV.name}", ha="center", fontsize=5.8, color=MUTED)
