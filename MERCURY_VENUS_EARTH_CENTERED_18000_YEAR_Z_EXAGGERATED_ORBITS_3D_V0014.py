# V0014
# Audit reference: BCE-safe JPL Horizons centered 3D orbit bundle using small explicit-Julian-date query batches and epoch-dependent Z-only exaggeration.
...
OUT = Path("/content/MERCURY_VENUS_EARTH_CENTERED_18000_YEAR_Z_EXAGGERATED_ORBITS_3D_V0014_OUTPUT")
PNG = OUT / "MERCURY_VENUS_EARTH_CENTERED_18000_YEAR_Z_EXAGGERATED_ORBITS_3D_V0014.png"
CSV = OUT / "MERCURY_VENUS_EARTH_CENTERED_18000_YEAR_Z_EXAGGERATED_ORBITS_3D_V0014.csv"
...
def z_scale_factor(year: int) -> float:
    return 1.0 if year == 0 else 10.0 * abs(float(year)) / 1000.0
...
def query_orbit(body: str, center_jd: float, period_days: float) -> tuple[np.ndarray, np.ndarray]:
    ...
    for start in range(0, len(epochs), QUERY_BATCH_SIZE):
        ...
        jd_part = np.asarray(table["datetime_jd"], dtype=float)
        xyz_part = np.column_stack([
            np.asarray(table["x"], dtype=float),
            np.asarray(table["y"], dtype=float),
            np.asarray(table["z"], dtype=float),
        ]) * AU_KM
        jd_parts.append(jd_part)
...
def make_plot(orbits: dict[str, list[dict]], summaries: list[dict]) -> None:
    ...
    ax.set_zlabel("Ecliptic Z (million km, exaggerated)", color="#E0E0E0", labelpad=12)
    ...
    info = [
        ...
        "Z factor: epoch 0 = 1×; each 1,000 years from zero adds 10×",
...
def main() -> None:
    ...
    section("COMMENTS")
    print("V0013 BCE range-query formatting is REJECTED and NOT USED.")
    ...
    print("NOT USED: analytic secular theory, VSOP, or non-JPL orbital extrapolation.")
    ...
    print("VERIFIED plotted Z = true JPL Z × epoch-dependent exaggeration factor")
