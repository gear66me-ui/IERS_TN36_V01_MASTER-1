# V0089D
# Audit reference: standalone Colab Python/Matplotlib/JPL widget; V0067 equations; contact-row color repair; no AI images.
...
ROOT = Path("/content")
OUT = ROOT / "VENUS_1769_V0089D_CONTACT_COLORS_OUTPUT"
PNG = OUT / "VENUS_1769_V0089D_CONTACT_COLORS_900DPI.png"
PDF = OUT / "VENUS_1769_V0089D_CONTACT_COLORS_VECTOR.pdf"
SVG = OUT / "VENUS_1769_V0089D_CONTACT_COLORS_VECTOR.svg"
CSV = OUT / "VENUS_1769_V0089D_CONTACT_COLORS.csv"
DPI = 900
...
def plot(pv: dict, va: dict, g: dict, max_resid: float) -> None:
    ...
    gs = fig.add_gridspec(1, 2, width_ratios=[2.05, 1], left=0.03, right=0.985, top=0.925, bottom=0.07, wspace=0.035)
    ...
    # Rows: 1-2 PV C1/C2 blue; 3 PV CA body; 4-5 PV C3/C4 teal; 6-7 Vardø C1/C2 blue; 8 Vardø CA body; 9-10 Vardø C3/C4 teal.
    style_table(ct, teal=(4, 5, 9, 10), blue=(1, 2, 6, 7), fs=5.55)

    fig.text(0.5, 0.018, "NO AI IMAGES — Python/Matplotlib only. Standalone JPL Horizons geometric ecliptic vector reconstruction; V0067 seconds-space geocentric CA restored.", ha="center", fontsize=6, color=MUTED)
...
def main() -> None:
    ...
    print("COMMENTS")
    print("Publication plot retained; Halley reduction rows removed from the plot; contact rows recolored by event class. No AI images.")
    m = master_frame()
    c = cache_build(m)
    pv = station(c, PV)
    va = station(c, VA)
    ...
    print("PASS: V0089D uses corrected V0067 equations; Halley audit rows removed from plot; contact row coloring updated.")
