# V0126B
# Audit reference: IERS-0012O engineering-style JPL track plot for the 1761 and 1769 IMCCE transit records.
...
VERSION = "IERS-0126B"
PROGRAM_NAME = "IERS_0126B_IMCCE_1761_1769_JPL_ENGINEERING_TRACKS.py"
...
OUTPUT_ROOT = Path("/content/IERS_TN36_V01_MASTER_OUTPUT/IMCCE_VENUS_CANON/PLOTS/V0126B")
TRACK_CSV = OUTPUT_ROOT / "IERS_0126B_IMCCE_1761_1769_JPL_TRACKS.csv"
RESULTS_CSV = OUTPUT_ROOT / "IERS_0126B_IMCCE_1761_1769_JPL_RESULTS.csv"
FIGURE_PNG = OUTPUT_ROOT / "IERS_0126B_IMCCE_1761_1769_JPL_ENGINEERING_TRACKS.png"
...
def resolve_workbook() -> Path:
    for path in (ORGANIZED_XLSX, MASTER_XLSX):
        if path.exists():
            return path
    raise FileNotFoundError("Run IERS-0125A before IERS-0126B; organized workbook not found.")
...
def seconds_to_clock(value: float) -> str:
    value %= 86400.0
    hour = int(value // 3600.0)
    minute = int((value % 3600.0) // 60.0)
    second = value % 60.0
...
def build_epoch_grid(record: dict[str, object], events: dict[str, float]) -> tuple[np.ndarray, np.ndarray]:
    minute_grid = np.arange(events["C1"], events["C4"] + STEP_DAYS * 0.25, STEP_DAYS)
    if minute_grid[-1] < events["C4"]:
        minute_grid = np.append(minute_grid, events["C4"])
    requested = np.unique(
        np.round(
            np.concatenate(
                [minute_grid, np.asarray(list(events.values())), np.asarray([float(record["jd_tdb"])])]
            ),
            12,
        )
    )
    return requested, minute_grid


def horizons_vectors(target_id: str, epochs: np.ndarray, prefix: str) -> pd.DataFrame:
    frames = []
    for start in range(0, len(epochs), QUERY_CHUNK):
        chunk = epochs[start : start + QUERY_CHUNK].astype(float).tolist()
        query = Horizons(id=target_id, location="500@399", epochs=chunk)
    ...
    output["epoch_key"] = np.round(output["jd_tdb"].to_numpy(dtype=float), 12)
    return output.drop_duplicates("epoch_key").sort_values("epoch_key").reset_index(drop=True)


def build_jpl_master(epochs: np.ndarray) -> pd.DataFrame:
    sun = horizons_vectors("10", epochs, "SUN")
    venus = horizons_vectors("299", epochs, "VENUS")
    ...
    if len(merged) != len(epochs):
        raise RuntimeError(f"JPL epoch mismatch: requested {len(epochs)}, received {len(merged)}")
    return merged.sort_values("jd_tdb").reset_index(drop=True)
...
def sky_offsets(master: pd.DataFrame, basis: tuple[np.ndarray, np.ndarray, np.ndarray, float, float]) -> tuple[np.ndarray, np.ndarray]:
    normal, xhat, yhat, _sun_radius, _venus_radius = basis
    x_values = []
    y_values = []
    for _, row in master.iterrows():
        venus_direction = unit(vector_from_row(row, "VENUS"))
        denominator = float(np.dot(venus_direction, normal))
        x_values.append(math.atan2(float(np.dot(venus_direction, xhat)), denominator) * ARCSEC_PER_RAD)
        y_values.append(math.atan2(float(np.dot(venus_direction, yhat)), denominator) * ARCSEC_PER_RAD)
...
def curvature_at_mid(time_values: np.ndarray, x_values: np.ndarray, y_values: np.ndarray) -> float:
    ...
    if denominator == 0.0:
        return 0.0
    return float(abs(x_prime * y_second - y_prime * x_second) / denominator)


def build_track(year: int, record: dict[str, object]) -> dict[str, object]:
    events = contact_jds(record)
    epochs, minute_grid = build_epoch_grid(record, events)
    master = build_jpl_master(epochs)
    basis = fixed_solar_basis(master, float(record["jd_tdb"]))
    x_arcsec, y_arcsec = sky_offsets(master, basis)
    ...
    event_jds = {"C1": events["C1"], "C2": events["C2"], "CA": closest_jd, "C3": events["C3"], "C4": events["C4"]}
    event_points = {
        event: np.asarray([float(spline_x(jd)), float(spline_y(jd))])
        for event, jd in event_jds.items()
    }

    minute_keys = set(np.round(minute_grid, 12))
    minute_master = master[master["epoch_key"].isin(minute_keys)].copy()
    points = minute_master[["x_solar_radii", "y_solar_radii"]].to_numpy(dtype=float)
    mean, direction, angle, rms = pca_metrics(points)
    linear_r2 = fit_r2(
        minute_master["jd_tdb"].to_numpy(dtype=float),
        points[:, 0],
        points[:, 1],
        1,
    )
    ...
    elapsed_seconds = (minute_master["jd_tdb"].to_numpy(dtype=float) - minute_master["jd_tdb"].iloc[0]) * 86400.0
    along = (points - mean) @ direction
    slope = float(np.polyfit(elapsed_seconds, along, 1)[0])
    ...
    return {
        ...
        "fit_speed_solar_radii_per_s": slope,
...
def summary_rows(tracks: list[dict[str, object]]) -> list[list[str]]:
    rows = []
    for track in tracks:
        year = track["year"]
...
def plot_engineering_tracks(tracks: list[dict[str, object]]) -> None:
    ...
    for track in tracks:
        ...
        dy = 0.045 if ca_point[1] >= 0.0 else -0.045
        add_label(axis, ca_point, f"{year} CA", 0.055, dy, color)
        for event, dx, dy_event in (
            ("C1", -0.115, 0.040),
            ("C2", -0.095, 0.030),
            ("C3", 0.055, -0.032),
            ("C4", 0.075, -0.043),
        ):
            add_label(axis, track["event_points"][event], f"{year} {event}", dx, dy_event, color)
    ...
    axis.set_title(
        ...
        "IMCCE contact epochs / JPL Horizons geocentric one-minute vectors — IERS-0012O style",
        color="#f8fdff",
        fontsize=9.0,
        pad=8,
    )
                ...
                writer.writerow(
                    {
                        ...
                        "source": "JPL_HORIZONS_GEOCENTER_VECTORS",
                    }
                )
            for event in ("C1", "C2", "CA", "C3", "C4"):
    ...
    with RESULTS_CSV.open("w", encoding="utf-8", newline="") as handle:
        ...
        for track in tracks:
            ...
            for quantity, value, unit_name, source in rows:
                writer.writerow([year, quantity, value, unit_name, source])
        writer.writerow(["COMPARISON", "halley_parallax_status", "NOT USED", "status", "1761 and 1769 are different epochs, not simultaneous observer tracks"])
...
def main() -> None:
    ...
    print("COMMENTS")
    print("V0126A is REJECTED. This version uses the exact IERS-0012O engineering visual architecture.")
    print("No AI images. Matplotlib only. IMCCE supplies contact epochs; JPL supplies every plotted one-minute position.")
    print("RESULTS")
    for track in tracks:
        ca = track["event_points"]["CA"]
        print(
            f"{track['year']} | angle={track['angle_deg']:.6f} deg | "
            f"CA=({ca[0]:+.6f}, {ca[1]:+.6f}) R_sun | "
            f"RMS={track['rms_solar_radii']:.9f} R_sun | "
    ...
    print("IMCCE contact epochs are used as event references; all plotted XY positions come from JPL Horizons vectors.")
    print("EQUATION STATUS")
    print("VERIFIED — tangent-plane projection, one-minute sampling, event splines, closest approach, PCA angle, RMS, R2, and curvature evaluated.")
