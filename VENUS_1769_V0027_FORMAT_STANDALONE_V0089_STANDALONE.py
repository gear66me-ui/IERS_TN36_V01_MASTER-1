# V0089
# Audit reference: Complete standalone Colab Python/Matplotlib/JPL widget; no external project Python imports, no runtime code fetching, no AI images.
...
ROOT = Path("/content")
OUT = ROOT / "VENUS_1769_V0089_STANDALONE_OUTPUT"
PNG = OUT / "VENUS_1769_V0089_STANDALONE.png"
CSV = OUT / "VENUS_1769_V0089_CONTACTS_GEOMETRY.csv"
...
def local_ca(c: dict, site: str, a: float, b: float) -> float:
    ...
    res = minimize_scalar(lambda s: sep_rad(vec(c, f"{site}_SUN", ref + s / 86400.0), vec(c, f"{site}_VENUS", ref + s / 86400.0)), bounds=(lo, hi), method="bounded", options={"xatol": 1e-4, "maxiter": 500})
    if not res.success:
...
def fit_track(points: np.ndarray) -> dict:
    ...
    return dict(mean=mean, direction=direction, normal=normal, angle=abs(angle), rms=rms)
...
def geo(c: dict) -> dict:
    jds = np.asarray(c["JD_TDB"], dtype=float)
    vals = np.array([sep_rad(vec(c, "GEOCENTER_SUN", jd), vec(c, "GEOCENTER_VENUS", jd)) for jd in jds])
    i = int(np.argmin(vals))
    res = minimize_scalar(lambda jd: sep_rad(vec(c, "GEOCENTER_SUN", jd), vec(c, "GEOCENTER_VENUS", jd)), bounds=(jds[max(0, i - 2)], jds[min(len(jds) - 1, i + 2)]), method="bounded")
    jd = float(res.x)
    ...
    return dict(jd=jd, utc=utc(jd), bas=bas, normal2=normal2, apbp_as=apbp_as, apbp_km=apbp_as * km_per_as, ab_km=ab_km, ab_as=ab_km / km_per_as)


def style_table(tab, gold=(), teal=(), fs=6.2, center_cols=()) -> None:
...
def draw_events(ax, st, evs, main: bool) -> None:
    ...
    for ev in evs:
        ...
        if main:
            ...
            dx, dy = ZOOM_LABEL_OVERRIDES.get((short, ev), (xbase[ev] * 0.4, ZOOM_PV_LABEL_Y if short == "PV" else ZOOM_VARDO_LABEL_Y))
...
def zoom_limits(items, evs):
    ...
    pts = np.array(pts); r = max(rs); m = r * 0.55 + 7
...
def plot(pv: dict, va: dict, g: dict, max_resid: float) -> None:
    plt.close("all")
    plt.rcParams.update({"figure.facecolor": BG, "axes.facecolor": BG, "savefig.facecolor": BG, "text.color": FG, "axes.labelcolor": FG, "xtick.color": MUTED, "ytick.color": MUTED, "font.family": "DejaVu Serif", "mathtext.fontset": "dejavuserif"})
    ...
    right = gs[0, 1].subgridspec(2, 1, height_ratios=[.47, .53], hspace=.095)
    resax, conax = fig.add_subplot(right[0, 0]), fig.add_subplot(right[1, 0])
    ...
    conax.axis("off"); conax.set_title("RECOMPUTED CONTACT TIMES — UTC", loc="left", fontsize=9, fontweight="bold", pad=5)
    ...
    fig.text(.5, .018, "NO AI IMAGES — Python/Matplotlib only. Standalone JPL Horizons geometric ecliptic vector reconstruction; no external project Python files referenced.", ha="center", fontsize=6, color=MUTED)
    fig.savefig(PNG, dpi=160, bbox_inches="tight", pad_inches=.02, facecolor=BG)
    display(Image(filename=str(PNG)))


def max_contact_resid(c: dict, items: tuple) -> float:
    vals = []
    for st in items:
        key = st["site"]["key"]
...
def write_csv(pv: dict, va: dict, g: dict, max_resid: float) -> None:
    ...
    rows.extend([dict(station="GEOMETRY", event="A_prime_B_prime_arcsec", utc="", jd_tdb=g["jd"], xi_arcsec=g["apbp_as"], eta_arcsec=np.nan), dict(station="GEOMETRY", event="AB_km", utc="", jd_tdb=g["jd"], xi_arcsec=g["ab_km"], eta_arcsec=max_resid)])
    pd.DataFrame(rows).to_csv(CSV, index=False)


def main() -> None:
    OUT.mkdir(parents=True, exist_ok=True)
    print("V0089 SOURCE AUDIT")
    print("standalone_no_external_project_python: PASS")
    print("no_ai_images: PASS")
    print("matplotlib_only_plotting: PASS")
    print("jpl_horizons_vectors: PASS")
    m = master_frame()
    c = cache_build(m)
    pv = station(c, PV)
    va = station(c, VA)
