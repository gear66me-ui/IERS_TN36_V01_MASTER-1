# V0089C
# Audit reference: standalone Colab Python/Matplotlib/JPL widget; V0067 seconds-space geocentric CA; publication audit tables; no AI images.
...
ROOT=Path("/content")
OUT=ROOT/"VENUS_1769_V0089C_PUBLICATION_AUDIT_OUTPUT"
PNG=OUT/"VENUS_1769_V0089C_PUBLICATION_AUDIT_900DPI.png"
PDF=OUT/"VENUS_1769_V0089C_PUBLICATION_AUDIT_VECTOR.pdf"
SVG=OUT/"VENUS_1769_V0089C_PUBLICATION_AUDIT_VECTOR.svg"
CSV=OUT/"VENUS_1769_V0089C_PUBLICATION_AUDIT.csv"
DPI=900
...
def geo(c):
    ...
    pi0=math.asin(EARTH_RADIUS_KM/IAU1976_AU_KM)*ARC; pi_epoch=math.asin(EARTH_RADIUS_KM/es_bar)*ARC; reduction=es_bar/IAU1976_AU_KM
    return dict(jd=jd,utc=utc(jd),bas=bas,normal2=normal2,apbp_as=apbp_as,apbp_km=apbp_as*km_per_as,ab_km=ab_km,ab_as=ab_km/km_per_as,ev_bar=ev_bar,vs_bar=vs_bar,es_bar=es_bar,km_per_as=km_per_as,pi0=pi0,pi_epoch=pi_epoch,reduction=reduction,pi_closure=pi_epoch*reduction)
...
def plot(pv,va,g,max_resid):
    ...
    fig=plt.figure(figsize=(18,10.5),facecolor=BG); gs=fig.add_gridspec(1,2,width_ratios=[2.05,1],left=.028,right=.985,top=.900,bottom=.065,wspace=.035)
    ...
    right=gs[0,1].subgridspec(3,1,height_ratios=[.31,.29,.40],hspace=.115); resax,audax,conax=fig.add_subplot(right[0,0]),fig.add_subplot(right[1,0]),fig.add_subplot(right[2,0])
    fig.suptitle("1769 Venus Transit Between Vardø, Norway, and Point Venus, Tahiti",fontsize=15,fontweight="bold",y=.953)
    ...
    der.axis("off"); der.text(.5,.895,"A′B′, AB, AND TRACK DERIVATION",ha="center",va="center",fontsize=8.2,fontweight="bold")
    ...
    resax.axis("off"); resax.set_title("IMPORTANT FIGURES OF MERIT",loc="left",fontsize=9,fontweight="bold",pad=5)
    rr=[["Quantity","Symbol","Value","Unit / status"],["IAU 1976 solar horizontal parallax","π₀",f"{g['pi0']:.6f}","arcsec"],["IAU 1976 full precision","π₀",f"{g['pi0']:.12f}","arcsec"],["A′B′ common-normal separation","A′B′",f"{g['apbp_as']:.6f}","arcsec"],["A′B′ common-normal separation","A′B′",f"{g['apbp_km']:,.6f}","km"],["AB projected baseline","AB",f"{g['ab_as']:.6f}","arcsec"],["AB projected baseline","AB",f"{g['ab_km']:,.6f}","km"],["Average track angle","ᾱ",f"{.5*(pv['fit']['angle']+va['fit']['angle']):.6f}","deg"],["Maximum contact residual","",f"{max_resid:.12f}","arcsec — PASS"]]
    rt=resax.table(cellText=rr,cellLoc="left",colWidths=[.49,.11,.22,.18],bbox=[0,0,1,.89]); rt.auto_set_font_size(False); style_table(rt,gold=(3,4,5,6),teal=(1,8),fs=5.9)
    audax.axis("off"); audax.set_title("ASTRONOMICAL UNITS, DIMENSIONS, AND HALLEY REDUCTION AUDIT",loc="left",fontsize=8.7,fontweight="bold",pad=4)
    ar=[["Quantity","Value","Unit / note"],["IAU 1976 AU = cτA",f"{IAU1976_AU_KM:,.6f}","km"],["JPL AU used for vector scaling",f"{JPL_AU_KM:,.6f}","km"],["Earth equatorial radius",f"{EARTH_RADIUS_KM:,.6f}","km"],["JPL Earth–Sun distance at CA",f"{g['es_bar']:,.6f}","km"],["JPL Earth–Venus axial distance",f"{g['ev_bar']:,.6f}","km"],["JPL Venus–Sun axial distance",f"{g['vs_bar']:,.6f}","km"],["km per arcsec at JPL ES",f"{g['km_per_as']:,.9f}","km/arcsec"],["Epoch parallax from JPL ES",f"{g['pi_epoch']:.12f}","arcsec"],["Reduction factor ES_JPL / AU_1976",f"{g['reduction']:.12f}","π₀ = π_epoch × factor"],["Closure",f"{g['pi_closure']:.12f}","arcsec — PASS"]]
    at=audax.table(cellText=ar,cellLoc="left",colWidths=[.48,.30,.22],bbox=[0,0,1,.88]); at.auto_set_font_size(False); style_table(at,gold=(8,9,10),teal=(1,4),fs=5.55,center_cols=(1,))
    conax.axis("off"); conax.set_title("CONTACT TIMES — UTC",loc="left",fontsize=9,fontweight="bold",pad=5); cr=[["Station","Event","UTC","Exact limb condition"]]
    for st in (pv,va):
        for ev in EVENTS:
    ...
    ct=conax.table(cellText=cr,cellLoc="left",colWidths=[.28,.12,.30,.30],bbox=[0,0,1,.91]); ct.auto_set_font_size(False); style_table(ct,teal=(1,2,3,6,7,8),fs=5.65)
...
def write_csv(pv,va,g,max_resid):
    rows=[]
    for st in (pv,va):
        for ev in EVENTS: rows.append(dict(section="contacts",station=st["site"]["label"],event=ev,utc=utc(st["events"][ev]),jd_tdb=st["events"][ev],value_arcsec=st["epts"][ev][0],value_km=st["epts"][ev][1]))
    metrics=[("pi0_arcsec",g["pi0"]),("pi_epoch_arcsec",g["pi_epoch"]),("reduction_ES_JPL_over_AU1976",g["reduction"]),("A_prime_B_prime_arcsec",g["apbp_as"]),("A_prime_B_prime_km",g["apbp_km"]),("AB_arcsec",g["ab_as"]),("AB_km",g["ab_km"]),("ES_bar_km",g["es_bar"]),("EV_bar_km",g["ev_bar"]),("VS_bar_km",g["vs_bar"]),("max_contact_residual_arcsec",max_resid)]
    for k,v in metrics: rows.append(dict(section="audit",station="GEOMETRY",event=k,utc=g["utc"],jd_tdb=g["jd"],value_arcsec=v,value_km=np.nan))
    pd.DataFrame(rows).to_csv(CSV,index=False)

def main():
    OUT.mkdir(parents=True,exist_ok=True)
    print("CODE INPUTS"); print(f"Version: {VERSION}"); print("JPL source: Horizons geometric ecliptic vectors fetched at runtime"); print(f"Raster DPI: {DPI}; vector outputs: PDF and SVG")
    print("COMMENTS"); print("Standalone publication audit: corrected V0089 plot with V0067 equations, contact-time table, A′B′/AB table, AU/dimension/Halley reduction table. No AI images.")
    ...
    print("RESULTS")
    print(f"π₀ IAU 1976: {g['pi0']:.6f} arcsec | full {g['pi0']:.12f}")
    ...
    print(f"JPL ES: {g['es_bar']:.6f} km | EV: {g['ev_bar']:.6f} km | VS: {g['vs_bar']:.6f} km")
    print(f"Reduction factor ES_JPL/AU_1976: {g['reduction']:.12f}")
    print(f"Halley reduction closure: π_epoch {g['pi_epoch']:.12f} × factor {g['reduction']:.12f} = {g['pi_closure']:.12f} arcsec")
    ...
    print("PAPER COMPARISON"); print("IAU 1976 π₀ listed first and rounded to six decimals on the plot table; full precision retained in audit table and CSV.")
    print("EQUATION STATUS"); print("PASS: V0089C uses V0067 seconds-space geocentric CA equations and recomputes all displayed values from JPL vectors.")
