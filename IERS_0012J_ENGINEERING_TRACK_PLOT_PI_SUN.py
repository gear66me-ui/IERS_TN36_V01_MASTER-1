# IERS-0012J
# Audit reference: GitHubDelivery@IERS-0012J; adds contact Venus disks, red/blue tracks, and plot summary table.
...
VERSION = "IERS-0012J"
PROGRAM_NAME = "IERS_0012J_ENGINEERING_TRACK_PLOT_PI_SUN.py"
...
TRACK_COLORS = {
    "Wellington NZ": "#ff3434",
    "Alaejos Spain": "#2f75ff",
}
...
def add_label(ax, xy, text, dx, dy, color):
    ax.annotate(
        ...
        ha="left",
        va="center",
        arrowprops=dict(arrowstyle="-", lw=0.22, color=color, shrinkA=0, shrinkB=2),
    )


def add_summary_table(ax, track_a, track_b, geometry):
    delta_angle = abs(track_a["track_angle_deg"] - track_b["track_angle_deg"])
    rows = [
        ["β Wellington", fmt(track_a["track_angle_deg"]), "deg"],
        ["β Alaejos", fmt(track_b["track_angle_deg"]), "deg"],
        ["Δβ", fmt(delta_angle), "deg"],
        ["π⊙", fmt(geometry["pi_sun_arcsec"]), "arcsec"],
        ["π⊙ Residual", fmt(geometry["pi_sun_residual_arcsec"]), "arcsec"],
        ["A′B′", fmt(geometry["A_prime_B_prime_arcsec"]), "arcsec"],
        ["A′B′", fmt(geometry["A_prime_B_prime_km"]), "km"],
        ["AB", fmt(geometry["AB_arcsec"]), "arcsec"],
        ["AB", fmt(geometry["AB_km"]), "km"],
    ]
    table = ax.table(
        cellText=rows,
        colLabels=["Quantity", "Value", "Unit"],
        loc="upper right",
        colWidths=[0.28, 0.20, 0.16],
        bbox=[0.604, 0.646, 0.374, 0.328],
    )
    ...
    ax.text(
        0.605,
        0.628,
        "A′B′ = solar-screen chord; AB = normal/projection baseline geometry.",
...
def write_event_csv(track_a, track_b, geometry, csv_path):
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        ...
        for key, label, unit in [
            ("A_prime_B_prime_arcsec", "A prime B prime", "arcsec"),
            ("A_prime_B_prime_km", "A prime B prime", "km"),
            ("AB_arcsec", "AB", "arcsec"),
            ("AB_km", "AB", "km"),
            ("pi_sun_arcsec", "pi_sun", "arcsec"),
            ("pi_sun_residual_arcsec", "pi_sun residual", "arcsec"),
        ]:
            writer.writerow([label, f"{geometry[key]:.6f}", unit])
...
def plot_engineering_track(cache, track_a, track_b, screen_jd, geometry, png_path):
    ...
    limb = Circle((0.0, 0.0), radius, fill=False, lw=0.38, ec="#66e8ff", alpha=0.95)
    ax.add_patch(limb)
    ...
    for track in [track_a, track_b]:
        ...
        for event in ["C1", "C2", "CA", "C3", "C4"]:
            ...
            disk = Circle((xy[0], xy[1]), r, fill=False, lw=0.24 if event != "CA" else 0.32, ec=color, alpha=0.92, zorder=2)
            ax.add_patch(disk)
    ...
    for event, dx, dy in [("C1", -48.0, 12.0), ("C2", -38.0, 9.0), ("C3", 20.0, -10.0), ("C4", 30.0, -13.0)]:
        ...
        add_label(ax, xy, event, dx, dy, "#8fb4c1")

    add_summary_table(ax, track_a, track_b, geometry)
