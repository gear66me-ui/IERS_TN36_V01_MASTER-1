# V0006
# Audit reference: standalone JPL Horizons single-orbit 3D heliocentric ecliptic visualization for Mercury, Venus, and Earth with perihelion and aphelion only.
...
OUT = Path("/content/MERCURY_VENUS_EARTH_SINGLE_3D_HELIOCENTRIC_ECLIPTIC_ORBITS_V0006_OUTPUT")
PNG = OUT / "MERCURY_VENUS_EARTH_SINGLE_3D_HELIOCENTRIC_ECLIPTIC_ORBITS_V0006.png"
CSV = OUT / "MERCURY_VENUS_EARTH_SINGLE_3D_HELIOCENTRIC_ECLIPTIC_ORBITS_V0006.csv"

PLANETS = {
    "Mercury": {"id": "199", "color": "#A970FF", "days": 90},
    "Venus": {"id": "299", "color": "#2F8DFF", "days": 226},
    "Earth": {"id": "399", "color": "#35C96B", "days": 366},
}
...
def query(body: str, stop_utc: str) -> tuple[np.ndarray, np.ndarray]:
    table = Horizons(
        id=body,
        id_type="majorbody",
        location=LOCATION,
    ...
    if len(jd) < 300 or not np.all(np.diff(jd) > 0.0):
...
def stop_from_days(days: int) -> str:
    start = Time(START_UTC, scale="utc")
    return Time(start.jd + float(days), format="jd", scale="utc").strftime("%Y-%m-%d %H:%M")
...
def sphere(radius: float, n_u: int = 56, n_v: int = 28) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
...
def make_plot(vectors: dict[str, np.ndarray], summaries: dict[str, dict]) -> None:
    ...
    for name in ("Mercury", "Venus", "Earth"):
        ...
        ax.plot(
            xyz_mkm[:, 0], xyz_mkm[:, 1], xyz_mkm[:, 2],
            color=cfg["color"], linewidth=1.15, alpha=0.95,
            label=f"{name} one orbit", zorder=3,
        )
        for label, key, marker in [
            ...
            ax.text(
                ...
                color=cfg["color"], fontsize=9.0, weight="bold", zorder=9,
            )

    set_equal_3d(ax, [vectors[name] / 1.0e6 for name in vectors])
    ax.view_init(elev=24.0, azim=38.0)
    ax.set_title(
        "Mercury, Venus, and Earth — Single Heliocentric Ecliptic Orbit",
    ...
    info = [
        "One JPL heliocentric ecliptic orbit per planet",
        "Only perihelion and aphelion are marked",
        "Sun sphere uses true radius at orbital scale",
    ]
    for name in ("Mercury", "Venus", "Earth"):
        row = summaries[name]
        info.append(
            ...
            f"Z {row['z_min_km']/1e6:.3f} to {row['z_max_km']/1e6:.3f} Mkm"
        )
    fig.text(
        0.025, 0.025, "\n".join(info),
...
def main() -> None:
    ...
    section("COMMENTS")
    print("One complete JPL orbit is plotted for each planet using its own orbital-duration window.")
    print("Only perihelion and aphelion points are marked; monthly markers are NOT USED.")
    print("Orbital precession over ten years is real but visually very small, so repeated traces nearly overlap.")
    ...
    for name, cfg in PLANETS.items():
        stop_utc = stop_from_days(int(cfg["days"]))
        print(f"DEBUG querying {name} through {stop_utc}", flush=True)
        jd, xyz = query(cfg["id"], stop_utc)
        jds[name] = jd
        vectors[name] = xyz
        summaries[name] = orbital_summary(name, jd, xyz)
        dates = Time(jd, format="jd", scale="tdb").utc.to_datetime()
        for index in range(len(jd)):
            rows.append({
                "planet": name,
                "utc": dates[index].strftime("%Y-%m-%d %H:%M:%S"),
                "x_km": xyz[index, 0],
                "y_km": xyz[index, 1],
                "z_km": xyz[index, 2],
            })
    ...
    print(f"PNG                                  {PNG}")
    print(f"CSV                                  {CSV}")
    print("Exactly three single-orbit traces    True")

    section("PAPER COMPARISON")
    print("NOT USED: published orbital elements or manually entered perihelion/aphelion distances.")
    print("All plotted coordinates and extrema are derived from JPL Horizons vectors.")

    section("EQUATION STATUS")
    print("VERIFIED radius = sqrt(X^2 + Y^2 + Z^2)")
    print("VERIFIED perihelion = minimum JPL heliocentric radius within each single-orbit interval")
    print("VERIFIED aphelion = maximum JPL heliocentric radius within each single-orbit interval")
