# V0092
# Audit reference: standalone JPL Horizons A-prime/B-prime fixed vector algorithm versus instantaneous tangent-velocity comparison plot.
...
OUT = Path("/content/VENUS_1769_APRIME_BPRIME_VECTOR_VS_INSTANT_V0092_OUTPUT")
PNG = OUT / "VENUS_1769_APRIME_BPRIME_VECTOR_VS_INSTANT_V0092.png"
CSV = OUT / "VENUS_1769_APRIME_BPRIME_VECTOR_VS_INSTANT_V0092.csv"
CONTACT_CSV = OUT / "VENUS_1769_APRIME_BPRIME_VECTOR_VS_INSTANT_CONTACTS_V0092.csv"
...
def geocentric_ca(c: dict[str, object]) -> float:
    ...
    ref = 0.5 * (lo_jd + hi_jd)
    lo = (lo_jd - ref) * 86400.0
    hi = (hi_jd - ref) * 86400.0
    res = minimize_scalar(
...
def fixed_apbp(c: dict[str, object], jd: float, g: dict[str, object]) -> dict[str, float]:
    pv = relative_xy_fixed(c, "POINT_VENUS", jd, g["basis"])
    va = relative_xy_fixed(c, "VARDO", jd, g["basis"])
    delta = va - pv
    return {"apbp_arcsec": float(np.dot(delta, g["normal"])), "along_arcsec": float(np.dot(delta, g["tangent"]))}
...
def analyze() -> tuple[pd.DataFrame, dict[str, dict[str, float]], dict[str, float | str]]:
    ...
    for prefix, col in (("fixed", "fixed_vector_apbp_arcsec"), ("instant", "instant_apbp_arcsec")):
        row = closest_sample(df, col)
        stats[f"{prefix}_sample_min_jd"] = float(row["jd"])
        stats[f"{prefix}_sample_min_utc"] = str(row["utc"])
        stats[f"{prefix}_sample_min_minute"] = float(row["minute"])
        stats[f"{prefix}_sample_min_value"] = float(row["value"])
...
def plot(df: pd.DataFrame, evs: dict[str, dict[str, float]], stats: dict[str, float | str]) -> None:
    ...
    ax_instant.scatter([imx], [imy], s=58, marker="X", color=RED, edgecolors=FG, linewidths=0.35, zorder=7)
    ax_instant.annotate(f"instant min A′B′ = {imy:.9f}″\n{stats['instant_sample_min_utc']}", xy=(imx, imy), xytext=(imx - 22, imy - 0.035), ha="right", va="top", fontsize=7.2, color=FG, arrowprops={"arrowstyle": "-", "color": FG, "linewidth": 0.32})
    ...
    fig.text(0.5, 0.043, f"File: {Path(__file__).name if '__file__' in globals() else 'VENUS_1769_APRIME_BPRIME_VECTOR_VS_INSTANT_V0092.py'} | Output: {PNG.name} | CSV: {CSV.name}", ha="center", fontsize=5.8, color=MUTED)
...
def main() -> None:
    ...
    print("No external project file is fetched; calculations are recomputed from JPL vectors.")
