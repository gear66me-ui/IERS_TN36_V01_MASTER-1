...
Part T — Refined colored three-plane sphere geometry
...
VERSION = "IERS-0001-T"
OUTPUT_PNG = "IERS_0001_T_JPL_THREE_PLANE_SPHERE_REFINED.png"
OUTPUT_PDF = "IERS_0001_T_JPL_THREE_PLANE_SPHERE_REFINED.pdf"
...
def draw_rotation_arc(
    ...
    samples = np.linspace(start_deg, end_deg, 160)
    points = np.array([polar_xy(angle, radius) for angle in samples])

    axis.plot(points[:, 0], points[:, 1], color=color, linewidth=1.05, zorder=6)
...
def render(result: dict[str, object]) -> None:
    ...
    draw_diameter(axis, ecliptic_angle, 1.12, COL_ECLIPTIC, 1.55, "-")
    draw_diameter(axis, celestial_angle, 1.12, COL_CELESTIAL, 1.35, "--")
    draw_diameter(axis, venus_angle, 1.12, COL_VENUS, 1.75, "-")

    axis.scatter([0.0], [0.0], s=22, color=COL_TEXT, zorder=9)

    # Left-side plane labels, aligned with their own diameters and kept clear of the lines.
    axis.text(
        -1.23,
        0.085,
        "ECLIPTIC PLANE",
    ...
    celestial_label_anchor = np.asarray(polar_xy(celestial_angle, -1.22))
    axis.text(
        celestial_label_anchor[0],
        celestial_label_anchor[1] - 0.095,
    ...
    venus_label_anchor = np.asarray(polar_xy(venus_angle, 1.12))
    axis.text(
        venus_label_anchor[0] + 0.03,
        venus_label_anchor[1] + 0.035,
        "VENUS TRANSIT PLANE",
        color=COL_VENUS,
        fontsize=13.5,
        ha="left",
