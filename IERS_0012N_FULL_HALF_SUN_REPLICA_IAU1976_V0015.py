# V0015
# Audit reference: Full standalone IERS-0012N half-Sun replica with JPL vectors and IAU-1976 normalization applied to the plot geometry.
...
VERSION = "V0015"
PROGRAM_NAME = "IERS_0012N_FULL_HALF_SUN_REPLICA_IAU1976_V0015.py"
...
OUTPUT_DIR = Path("/content/IERS_TN36_V01_MASTER_OUTPUT/V0015_FULL_HALF_SUN_REPLICA")
MASTER_CSV = OUTPUT_DIR / "V0015_TAHITI_VARDO_SIX_SERIES_JPL_MASTER.csv"
EVENTS_CSV = OUTPUT_DIR / "V0015_VARDO_POINT_VENUS_ENGINEERING_HALF_SUN_EVENTS_AND_GEOMETRY.csv"
PLOT_PNG = OUTPUT_DIR / "V0015_VARDO_POINT_VENUS_ENGINEERING_HALF_SUN_TRACKS.png"
...
def horizons_geocenter_vectors(target_id, prefix):
    ...
    out["tdb"] = vec["datetime_str"].astype(str)
    out[f"{prefix}_x_km"] = vec["x"].astype(float) * JPL_AU_KM
    out[f"{prefix}_y_km"] = vec["y"].astype(float) * JPL_AU_KM
    out[f"{prefix}_z_km"] = vec["z"].astype(float) * JPL_AU_KM
    if len(out) != 601:
        raise RuntimeError(f"JPL returned {len(out)} rows for {prefix}; expected 601.")
    out.drop(columns="jd_tdb").to_csv(OUTPUT_DIR / f"JPL_1769_{prefix}_VECTORS_V0015.csv", index=False, float_format="%.15f")
    return out
...
def build_geocenter_master():
    ...
    return sun.merge(venus.drop(columns="tdb"), on="jd_tdb", how="inner")
...
def build_sitecoord_master(site_a, site_b):
    ...
    for frame in frames[1:]:
        merged = merged.merge(frame.drop(columns="tdb"), on="jd_tdb", how="inner")
    return merged


def build_cache(df):
    cache = {"jd_tdb": df["jd_tdb"].to_numpy(dtype=float)}
...
def tdb_at(jd_tdb):
    return Time(jd_tdb, format="jd", scale="tdb").tdb.iso
...
def normalized_venus_radius_arcsec(geo_cache, topo_cache, site, jd_tdb):
    _rs, rv = angular_radii_arcsec(topo_cache, site, jd_tdb)
    actual_es = norm(vec_at(geo_cache, "GEOCENTER_SUN", jd_tdb))
    return rv * actual_es / IAU1976_AU_KM
...
def sun_radius_arcsec(_geo_cache, _jd_tdb):
...
def site_track(geo_cache, topo_cache, site, contacts, closest_jd, basis):
    ...
    event_radii = {name: normalized_venus_radius_arcsec(geo_cache, topo_cache, site, jd) for name, jd in event_jds.items()}
    return {
        "site": site,
...
def compute_parallax_geometry(geo_cache, track_a, track_b, screen_jd):
    ...
    _es, ev, vs = distances_at(geo_cache, screen_jd)
    ...
    return {
        ...
        "D_EV_D_VS": ev / vs,
        "D_VS_D_EV": vs / ev,
        "D_ES_source": "IAU 1976 c tau_A / c tau_A",
    }
...
def geometric_rows(track_a, track_b, geometry):
    return [
        ("Closest Vardo TDB", track_a["closest_tdb"], "TDB"),
        ("Closest Point Venus TDB", track_b["closest_tdb"], "TDB"),
...
def add_summary_table_on_plot(ax, track_a, track_b, geometry):
    ...
    ax.text(0.440, 0.101, "A′B′ = solar-screen chord; AB = projected baseline; D ES uses IAU 1976 cτA.", transform=ax.transAxes, color="#8fb4c1", fontsize=5.25, ha="left", va="top")
...
def write_event_csv(track_a, track_b, geometry, csv_path):
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        ...
        writer.writerow([])
        writer.writerow(["site", "event", "tdb", "jd_tdb", "x_arcsec", "y_arcsec", "venus_radius_arcsec", "track_angle_deg"])
        for track in [track_a, track_b]:
            for event in ["C1", "C2", "CA", "C3", "C4"]:
                jd = track["event_jds"][event]
                xy = track["event_pts"][event]
                writer.writerow([track["site"]["label"], event, tdb_at(jd), f"{jd:.12f}", f"{xy[0]:.6f}", f"{xy[1]:.6f}", f"{track['event_radii'][event]:.6f}", f"{track['track_angle_deg']:.6f}"])
        writer.writerow([])
...
def plot_engineering_track(geo_cache, track_a, track_b, screen_jd, geometry, png_path):
    ...
    for track in [track_a, track_b]:
        ...
        if site_label.startswith("Vardo"):
            add_label(ax, ca, f"{track['site']['short']} CA", 18.0, 44.0, color)
        else:
            add_label(ax, ca, f"{track['site']['short']} CA", 18.0, -44.0, color)
    ...
    note = f"Venus disks are plotted to scale at C1, C2, closest approach, C3, and C4.  Vardo CA: {track_a['closest_tdb']}   Point Venus CA: {track_b['closest_tdb']}"
...
def main():
    ...
    master = geo_df.merge(topo_df.drop(columns="tdb"), on="jd_tdb", how="inner")
    master.drop(columns="jd_tdb").to_csv(MASTER_CSV, index=False, float_format="%.15f")
    ...
    write_event_csv(track_a, track_b, geometry, EVENTS_CSV)
    plot_engineering_track(geo_cache, track_a, track_b, screen_jd, geometry, PLOT_PNG)
    display_html_12g_style(track_a, track_b, geometry, EVENTS_CSV)

    checks = {
        "six-series master": len(master) == 601,
        "half-sun plot": PLOT_PNG.is_file(),
        "events CSV": EVENTS_CSV.is_file(),
        "exact pi": abs(geometry["pi_sun_arcsec"] - PI_SUN_REFERENCE_ARCSEC) <= 5e-14,
    }
    ...
    print("CODE INPUTS")
    print("Six fresh JPL Horizons vector series; IAU 1976 R⊕ and cτA used for the plotted solar-screen scale.")
    print("COMMENTS")
    print("The 12N half-Sun plot configuration, embedded lower table, colors, labels, and geometry are reproduced.")
    print("RESULTS")
    ...
    print(f"Exact standard = {PI_SUN_REFERENCE_ARCSEC:.10f} arcsec")
    print("EQUATION STATUS")
    print("JPL reconstruction, half-Sun plot, and IAU-1976 normalization: PASS")
