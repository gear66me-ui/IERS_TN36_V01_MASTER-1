# IERS-0012E
# Audit reference: GitHubDelivery@IERS-0012E; based on IERS-0012D with dashboard-only report styling changes.
...
VERSION = "IERS-0012E"
PROGRAM_NAME = "IERS_0012E_ANTIPODAL_CITY_PAIR_DASHBOARD_PI_SUN.py"
...
def build_dashboard_sections(parallax, track_rows):
    trig_cards = [
        ("Closest UTC", parallax["CA_utc"], "UTC"),
        ("JD TDB", parallax["CA_jd_tdb"], "d"),
        ("Mean θ", parallax["theta_mean_deg"], "deg"),
        ("Common tangent θ", parallax["common_angle_deg"], "deg"),
        ("A′B′ chord", parallax["A_prime_B_prime_arcsec"], "arcsec"),
        ("A′B′ screen", parallax["A_prime_B_prime_km"], "km"),
    ...
    return trig_cards, solution_cards, track_rows_out
...
def metric_card(label, value, unit, mode="standard"):
    value_text = fmt(value, 6)
    mode_class = " metric-accent" if mode == "accent" else ""
    return f"""
      <div class="metric-card{mode_class}">
        <div class="metric-label">{html_escape(label)}</div>
        <div class="metric-value">{html_escape(value_text)}</div>
        <div class="metric-unit">{html_escape(unit)}</div>
      </div>
    """


def metric_grid(cards, accent_labels):
    blocks = []
    for label, value, unit in cards:
        mode = "accent" if label in accent_labels else "standard"
        blocks.append(metric_card(label, value, unit, mode))
    return "<div class='metric-grid'>" + "\n".join(blocks) + "</div>"


def html_track_table(rows):
    headers = ["track", "lat_deg", "lon_deg", "angle_deg", "linear_R2", "quad_R2", "cubic_R2"]
    out = ["<table class='track-table'>"]
    out.append("<thead><tr>" + "".join(f"<th>{html_escape(h)}</th>" for h in headers) + "</tr></thead>")
    out.append("<tbody>")
    for row in rows:
        out.append("<tr>")
        for i, cell in enumerate(row):
            text = str(cell) if i == 0 else fmt(cell, 6)
            css = " class='value-cell'" if i > 0 else ""
            out.append(f"<td{css}>{html_escape(text)}</td>")
        out.append("</tr>")
...
def display_html_report(parallax, track_rows, csv_path, title):
    ...
    trig_cards, solution_cards, track_rows_out = build_dashboard_sections(parallax, track_rows)
    ...
    .track-table th {{
        color:#66e8ff;
        background:#0a1a22;
        border-bottom:1px solid #1d3d4a;
        padding:7px 8px;
        text-align:left;
        text-transform:uppercase;
    ...
    .track-table td {{
        border-top:1px solid #102630;
        padding:7px 8px;
        color:#dff8ff;
    }}
    .track-table .value-cell {{
        color:#ffc861;
        text-align:right;
    }}
    .dashboard-note {{
    ...
    <div class="iers-shell">
      <div class="dashboard-title"><span>{html_escape(VERSION)}</span> — {html_escape(title)}</div>

      <div class="section-title">01 — Trigonometry dashboard</div>
      {metric_grid(trig_cards, {"Normal separation ρ", "Projected baseline", "D_EV / D_VS"})}

      <div class="section-title">02 — π⊙ solution dashboard</div>
      {metric_grid(solution_cards, {"π⊙ computed", "π⊙ residual"})}

      <div class="section-title">03 — Track fit table</div>
      {html_track_table(track_rows_out)}

      <div class="dashboard-note">CSV: {html_escape(csv_path)}</div>
    </div>
    """
    display(HTML(html))
    return True
...
def write_csv(parallax, track_rows, csv_path):
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow([VERSION, "MIDNIGHT LAB DASHBOARD REPORT"])
        writer.writerow([])
        writer.writerow(["TRIGONOMETRY DASHBOARD"])
        writer.writerow(["quantity", "value", "unit"])
        for row in trig_cards:
            writer.writerow(row)
        writer.writerow([])
        writer.writerow(["π⊙ SOLUTION DASHBOARD"])
        writer.writerow(["quantity", "value", "unit"])
        for row in solution_cards:
            writer.writerow(row)
        writer.writerow([])
...
def print_plain_report(parallax, track_rows, csv_path, label):
    trig_cards, solution_cards, track_rows_out = build_dashboard_sections(parallax, track_rows)
    print_plain_section(f"TRIGONOMETRY DASHBOARD — {label}", [("quantity", "value", "unit")] + trig_cards)
    print_plain_section(f"π⊙ SOLUTION DASHBOARD — {label}", [("quantity", "value", "unit")] + solution_cards)
    print_plain_section(
        f"TRACK FIT TABLE — {label}",
...
def run_pair(pair, out_dir):
    ...
    iers_csv = os.path.join(out_dir, f"{VERSION}_{safe_title}_PI_SUN_IERS_GCRS_DASHBOARD.csv")
    write_csv(parallax, track_rows, iers_csv)
    rendered = display_html_report(parallax, track_rows, iers_csv, pair["title"] + " — IERS GCRS")
    if not rendered:
        print_plain_report(parallax, track_rows, iers_csv, "IERS GCRS")
    ...
    sitecoord_csv = os.path.join(out_dir, f"{VERSION}_{safe_title}_PI_SUN_SITE_COORD_DASHBOARD.csv")
    ...
    if not rendered:
        print_plain_report(sitecoord_parallax, sitecoord_track_rows, sitecoord_csv, "HORIZONS SITE_COORD")
