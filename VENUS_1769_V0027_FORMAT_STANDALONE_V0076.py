...
'''
    if old not in source:
        if "VENUS_PAINT_LINE_FACTOR" in source:
            return source
        raise RuntimeError("Venus disk block was not found; no unsafe partial patch applied.")
    return source.replace(old, new, 1)


def inject_venus_paint_constant(source: str) -> str:
    marker = f"SUN_FILL_ALPHA = {SOLAR_FILL_ALPHA:.3f}"
    replacement = marker + f"\nVENUS_PAINT_LINE_FACTOR = {VENUS_PAINT_LINE_FACTOR:.6f}"
    if replacement in source:
        return source
    if marker not in source:
        raise RuntimeError("Solar constants marker was not found; no unsafe partial patch applied.")
...
def build_v0076_source() -> str:
    ...
    source = inject_venus_paint_constant(source)
...
def main() -> None:
    source = build_v0076_source()
    namespace = {"__name__": "__main__", "__file__": "VENUS_1769_V0027_FORMAT_STANDALONE_V0076.py"}
    exec(compile(source, "VENUS_1769_V0027_FORMAT_STANDALONE_V0076.py", "exec"), namespace, namespace)
