...
EXPANDED_OUT = Path("/content/VENUS_1769_V0027_FORMAT_STANDALONE_V0087_EXPANDED.py")
...
def promote_to_v0087(source: str) -> str:
...
def audit_source(source: str) -> None:
    checks = {
        "version_v0087": "# V0087" in source and 'VERSION = "V0087"' in source,
    ...
    print("V0087 SOURCE AUDIT")
    for name, ok in checks.items():
        print(f"{name}: {'PASS' if ok else 'FAIL'}")
    if failed:
        raise RuntimeError(f"V0087 audit failed: {failed}")
    print("V0087 audit complete: PASS")


def main() -> None:
    source = build_v0085_expanded_source()
    source = swap_zoom_windows_only(source)
    source = promote_to_v0087(source)
    audit_source(source)
