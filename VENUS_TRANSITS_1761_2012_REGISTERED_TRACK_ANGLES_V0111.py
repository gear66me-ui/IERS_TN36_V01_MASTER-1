...
LOCATION, REFPLANE, REFSYSTEM, ABERRATIONS, STEP = "@0", "earth", "ICRF", "geometric", "1m"
...
def query(body: str,start: str,stop: str)->Series:
    t=Horizons(id=body,id_type="majorbody",location=LOCATION,epochs={"start":start,"stop":stop,"step":STEP}).vectors(refplane=REFPLANE,refsystem=REFSYSTEM,aberrations=ABERRATIONS)
    jd=np.asarray(t["datetime_jd"],float)
    r=np.column_stack([np.asarray(t[k],float) for k in ("x","y","z")])*AU_KM
    if len(jd)<100 or not np.all(np.diff(jd)>0): raise RuntimeError(f"REJECTED JPL grid for {body}")
    return Series(jd,r)
