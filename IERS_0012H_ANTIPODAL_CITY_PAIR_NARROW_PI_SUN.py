# IERS-0012H
# Audit reference: GitHubDelivery@IERS-0012H; observer-specific closest approaches added and narrow 12D table retained.
...
VERSION = "IERS-0012H"
PROGRAM_NAME = "IERS_0012H_ANTIPODAL_CITY_PAIR_NARROW_PI_SUN.py"
...
def find_geocenter_closest(cache):
    jds = cache["jd_tdb"]
    vals = [geocenter_sep_arcsec(cache, jd) for jd in jds]
    i = int(np.argmin(vals))
    a = jds[max(0, i - 3)]
    b = jds[min(len(jds) - 1, i + 3)]
    res = minimize_scalar(lambda jd: geocenter_sep_arcsec(cache, jd), bounds=(a, b), method="bounded", options={"xatol": 1e-13})
    return float(res.x)
...
def compute_iers_guideline_method(cache):
    ca_jd_a = find_site_closest(cache, SITE_A)
    ca_jd_b = find_site_closest(cache, SITE_B)
    screen_jd = 0.5 * (ca_jd_a + ca_jd_b)
    ...
    beta_mean = 0.5 * (track_rows[0]["angle_deg"] + track_rows[1]["angle_deg"])
    ...
    parallax = {
        "closest_a_utc": utc_at(cache, ca_jd_a),
        "closest_b_utc": utc_at(cache, ca_jd_b),
        "closest_a_jd_tdb": ca_jd_a,
        "closest_b_jd_tdb": ca_jd_b,
        "screen_reference_utc": utc_at(cache, screen_jd),
        "screen_reference_jd_tdb": screen_jd,
...
def sitecoord_sep_arcsec(topo_cache, site, jd_tdb):
    key = site["key"]
    return angular_sep_arcsec(vec_at(topo_cache, f"{key}_SUN", jd_tdb), vec_at(topo_cache, f"{key}_VENUS", jd_tdb))


def find_sitecoord_closest(topo_cache, site):
    jds = topo_cache["jd_tdb"]
    vals = [sitecoord_sep_arcsec(topo_cache, site, jd) for jd in jds]
    i = int(np.argmin(vals))
    a = jds[max(0, i - 3)]
    b = jds[min(len(jds) - 1, i + 3)]
    res = minimize_scalar(lambda jd: sitecoord_sep_arcsec(topo_cache, site, jd), bounds=(a, b), method="bounded", options={"xatol": 1e-13})
    return float(res.x)
...
def compute_iers_guideline_method_sitecoord(geo_cache, topo_cache):
    ca_jd_a = find_sitecoord_closest(topo_cache, SITE_A)
    ca_jd_b = find_sitecoord_closest(topo_cache, SITE_B)
    screen_jd = 0.5 * (ca_jd_a + ca_jd_b)
    ...
    return {
        "closest_a_utc": utc_at(geo_cache, ca_jd_a),
        "closest_b_utc": utc_at(geo_cache, ca_jd_b),
        "closest_a_jd_tdb": ca_jd_a,
        "closest_b_jd_tdb": ca_jd_b,
        "screen_reference_utc": utc_at(geo_cache, screen_jd),
        "screen_reference_jd_tdb": screen_jd,
        "A_prime_B_prime_arcsec": abp_arcsec,
        "A_prime_B_prime_km": abp_km,
        "baseline_proj_km": baseline_proj_km,
...
def build_trig_rows(parallax, track_rows):
    return [
        ...
        ("β Average", parallax["beta_mean_deg"], "deg"),
    ]


def build_solution_rows(parallax):
    return [
        ("Closest Wellington UTC", parallax["closest_a_utc"], "UTC"),
        ("Closest Alaejos UTC", parallax["closest_b_utc"], "UTC"),
