# V0117
# Audit reference: fresh NASA/JPL transit geometry, calendar-month display mapping, full circular solar limb, explicit Colab PNG rendering.
...
PNG_NAME = "VENUS_1769_GEOCENTRIC_CALENDAR_MONTHS_V0117.png"
CSV_NAME = "VENUS_1769_GEOCENTRIC_CALENDAR_MONTHS_V0117.csv"
...
def fetch_vectors(target: str) -> tuple[np.ndarray, np.ndarray]:
    request = urllib.request.Request(
        build_url(target),
        headers={"User-Agent": "V0117-JPL-Horizons-Audit"},
    )
...
def add_full_solar_limb(ax: plt.Axes, center_date: datetime) -> None:
    fig = ax.figure
    fig.canvas.draw()
    center_num = mdates.date2num(center_date)
    p0 = ax.transData.transform((center_num, 0.0))
    p1 = ax.transData.transform((center_num, SOLAR_RADIUS_ARCSEC))
    radius_px = abs(float(p1[1] - p0[1]))
    radius_pt = radius_px * 72.0 / fig.dpi
    drawing = DrawingArea(2.0 * radius_pt, 2.0 * radius_pt, 0.0, 0.0)
    limb = Circle(
        (radius_pt, radius_pt),
        radius_pt,
        fill=False,
...
def main() -> None:
    ...
    print("The full circular solar limb is centered at the interpolated closest approach.")
    ...
    add_full_solar_limb(ax, ca_datetime)
    ...
    print("PASS: the full circular solar limb is centered at closest approach and y=0.")
