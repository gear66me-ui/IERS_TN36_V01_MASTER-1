# V0015
# Audit reference: BCE-safe JPL Horizons centered 3D orbit bundle with 2x-per-millennium Z-only exaggeration.
...
Z_PER_MILLENNIUM = 2.0
OUT = Path("/content/MERCURY_VENUS_EARTH_CENTERED_18000_YEAR_Z_2X_ORBITS_3D_V0015_OUTPUT")
PNG = OUT / "MERCURY_VENUS_EARTH_CENTERED_18000_YEAR_Z_2X_ORBITS_3D_V0015.png"
CSV = OUT / "MERCURY_VENUS_EARTH_CENTERED_18000_YEAR_Z_2X_ORBITS_3D_V0015.csv"
...
def epoch_center_jd(year: int) -> float:
    if not HORIZONS_MIN_YEAR <= year <= HORIZONS_MAX_YEAR:
        raise ValueError(
            f"REJECTED epoch {year}: Horizons limit is "
            f"{HORIZONS_MIN_YEAR} to {HORIZONS_MAX_YEAR}"
        )
...
def z_scale_factor(year: int) -> float:
    if year == 0:
        return 1.0
    return Z_PER_MILLENNIUM * abs(float(year)) / 1000.0
...
def orbit_summary(name: str, year: int, jd: np.ndarray, xyz: np.ndarray) -> dict:
    ...
    return {
        ...
        "z_min_plot_km": float(np.min(xyz[:, 2]) * factor),
        "z_max_plot_km": float(np.max(xyz[:, 2]) * factor),
    }
...
def make_plot(orbits: dict[str, list[dict]], summaries: list[dict]) -> None:
    ...
    ax.set_title(
        ...
        "Astronomical years −9000 to +9000; Z exaggerated 2× per millennium",
        color="white",
        fontsize=20,
        weight="bold",
        pad=30,
    ...
    ax.set_zlabel("Ecliptic Z (million km, epoch-exaggerated)", color="#E0E0E0", labelpad=12)
    ...
    info = [
        ...
        "Z factor: epoch 0 = 1×; each 1,000 years from zero adds 2×",
        "±1000→2×, ±5000→10×, ±9000→18×",
...
def main() -> None:
    ...
    print(f"Samples per complete orbit           {SAMPLES_PER_ORBIT}")
    print(f"Explicit-JD query batch size         {QUERY_BATCH_SIZE}")
    print(f"Orbit linewidth                      {ORBIT_LINEWIDTH:.2f}")
    print(f"Z increase per millennium            {Z_PER_MILLENNIUM:.1f}x")
    print(f"Reference center                     Sun, {LOCATION}")
    print(f"Reference plane                      {REFPLANE}")
    print(f"Output                               {OUT}")

    section("COMMENTS")
    print("No AI images are generated or used.")
    print("All scientific coordinates are downloaded from JPL Horizons.")
    print("Each orbit is queried through two small explicit-Julian-date batches.")
    print("X and Y are unchanged JPL coordinates; only plotted Z is exaggerated by epoch.")
    print("The prior 10x-per-millennium Z scaling is REJECTED and NOT USED.")
    ...
    for year in EPOCH_YEARS:
        ...
        for name, cfg in PLANETS.items():
            print(f"DEBUG querying {name} complete orbit at astronomical year {year:+d}", flush=True)
    ...
    for name in ("Mercury", "Venus", "Earth"):
        ...
        for row in rows:
            print(
                f"{name:7s} {row['epoch_year']:+6d}  Z×{row['z_scale_factor']:5.1f}  "
                f"P {row['perihelion_km']:15.6f} km  "
                f"A {row['aphelion_km']:15.6f} km  "
                f"i {row['inclination_deg']:9.6f} deg"
            )
    ...
    print("NOT USED: analytic secular theory, VSOP, AI imagery, or non-JPL orbital extrapolation.")
    print("The ±10000-year endpoints are REJECTED because the online Horizons limit is ±9999.")
    ...
    print("VERIFIED plotted Z = true JPL Z × 2 × |epoch year| / 1000, with epoch 0 fixed at 1×")
    print("VERIFIED X and Y are unchanged JPL heliocentric ecliptic coordinates")
