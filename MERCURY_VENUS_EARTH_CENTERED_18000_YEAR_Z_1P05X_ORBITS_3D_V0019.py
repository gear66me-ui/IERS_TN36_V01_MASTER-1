# V0019
# Audit reference: standalone BCE-safe JPL Horizons centered 3D orbit bundle with uniform Z x 1.05 scaling and minimal notebook output.
...
Z_SCALE = 1.05
OUT = Path("/content/MERCURY_VENUS_EARTH_CENTERED_18000_YEAR_Z_1P05X_ORBITS_3D_V0019_OUTPUT")
PNG = OUT / "MERCURY_VENUS_EARTH_CENTERED_18000_YEAR_Z_1P05X_ORBITS_3D_V0019.png"
CSV = OUT / "MERCURY_VENUS_EARTH_CENTERED_18000_YEAR_Z_1P05X_ORBITS_3D_V0019.csv"
...
def orbit_summary(name: str, year: int, jd: np.ndarray, xyz: np.ndarray) -> dict:
    ...
    return {
        ...
        "sample_count": len(jd),
        "perihelion_index": int(np.argmin(radius)),
        "aphelion_index": int(np.argmax(radius)),
    }
...
def make_plot(orbits: dict[str, list[dict]], summaries: list[dict]) -> None:
    ...
    ax.set_title(
        ...
        "Astronomical years −9000 to +9000; uniform Z × 1.05",
        color="white",
        fontsize=20,
        weight="bold",
        pad=30,
    ...
    ax.set_zlabel("Ecliptic Z (million km, uniformly ×1.05)", color="#E0E0E0", labelpad=12)
